(() => {
    ...
    const els = {
        ...
        importModal: document.getElementById("importModal")
            ? new bootstrap.Modal(document.getElementById("importModal"))
            : null,
    ...
    const state = {
        ...
        lookupData: { categories: [], brands: [], suppliers: [] },
        selectedCategories: new Map(),
        selectedBrands: new Map(),
        selectedSuppliers: new Map(),
        autocomplete: {
            category: { selectedIndex: -1, suggestions: [], isOpen: false },
            brand: { selectedIndex: -1, suggestions: [], isOpen: false },
            supplier: { selectedIndex: -1, suggestions: [], isOpen: false },
        },
    };
    ...
    function applyColumnVisibility() {
        ...
        document
            .querySelectorAll("th[data-col], td[data-col]")
            .forEach((el) => {
    ...
    function updateUrl() {
        ...
        if (els.hdrSearch && els.hdrSearch.value.trim())
            p.set("q", els.hdrSearch.value.trim());
        if (state.sortBy) {
        ...
        const brandId =
            els.brandAutocomplete?.dataset.selectedValue || els.brandId.value;
        const supplierId =
    ...
    async function fetchList() {
        ...
        if (els.hdrSearch && els.hdrSearch.value.trim())
            params.set("search", els.hdrSearch.value.trim());
        ...
        try {
            ...
            if (
                ...
                console.log(
                    "Empty items but more pages exist, going to previous page"
                );
            ...
            renderTable(items);
            renderPagination(
                data.currentPage ?? data.page ?? state.page,
                data.totalPages ??
                    Math.ceil((data.totalElements || 0) / state.size)
            );
    ...
    function renderTable(items) {
        ...
        <tr data-id="${p.id}" tabindex="0" aria-expanded="${isExpanded}">
          ...
          <td class="fw-semibold" data-col="name" title="${fmt(p.name)}">${fmt(
                p.name
            )}</td>
          <td data-col="category" title="${fmt(p.category?.name)}">${fmt(
                p.category?.name
            )}</td>
          <td data-col="brand" title="${fmt(p.brand?.name)}">${fmt(
                p.brand?.name
            )}</td>
          <td data-col="supplier" title="${fmt(p.supplier?.name)}">${fmt(
                p.supplier?.name
            )}</td>
          <td data-col="sellingPrice" class="text-end" title="${fmt(
              p.sellingPrice
          )}">${
                p.sellingPrice
        ...
        <td colspan="10">
          <div class="expanded-wrapper">
            ...
            <ul class="nav nav-tabs" id="productTabs-${p.id}" role="tablist">
              ...
              <li class="nav-item" role="presentation">
                <button class="nav-link" id="inventory-tab-${
                    p.id
                }" data-bs-toggle="tab"
              ...
              <li class="nav-item" role="presentation">
                <button class="nav-link" id="pricing-tab-${
                    p.id
                }" data-bs-toggle="tab"
              ...
              <div class="tab-pane fade" id="description-${
                ...
                <div class="tab-content-wrapper">
                  <div class="description-content">
                    ...
                    ${
                        p.description
                            ? `<div class="description-text">${formatDescription(
                                  p.description
                              )}</div>`
                    ...
                    <div class="col-md-6">
                      ...
                      <div class="inventory-details">
                        <div class="field-item mb-3">
                          ...
                          <div class="field-value fs-4 fw-bold ${
                              stockStatus.class
                          }">${fmt(p.onHand)} units</div>
                        </div>
                        <div class="field-item mb-3">
                    ...
                    <div class="col-md-6">
                      ...
                      <div class="inventory-settings">
                        ...
                        <div class="alert alert-info">
                          ...
                          ${
                              stockStatus.action
                                  ? `<br><strong>Recommended Action:</strong> ${stockStatus.action}`
                                  : ""
                          }
                        </div>
                      </div>
                    ...
                    <div class="col-md-6">
                      ...
                      <div class="pricing-analysis">
                        ${
                            ...
                            <strong>Margin:</strong> ${calcMarginStr(
                                p.sellingPrice,
                                p.costPrice
                            )}
                          </div>`
    ...
    function setSaving(saving) {
        ...
        if (els.btnSaveNew) {
            els.btnSaveNew
                ...
                .classList.toggle("d-none", saving);
            els.btnSaveNew
    ...
    async function save() {
        ...
        if (!payload.name) {
            els.modalErr.textContent = "Product name is required";
    ...
    async function doDelete(id) {
        ...
        const resp = await fetch(`${api.base}/${id}`, {
            method: "DELETE",
            headers: api.headers(),
        }).then(authGuard);
        if (resp.ok) {
    ...
    function renderChips() {
        ...
        if (els.hdrSearch && els.hdrSearch.value.trim()) {
            chips.push(
                chip("search", "Search", els.hdrSearch.value.trim(), () => {
                    els.hdrSearch.value = "";
                    fetchList();
                })
            );
        ...
        state.selectedCategories.forEach((item, id) => {
            chips.push(
                chip("category", "Category", item.name, () => {
        ...
        state.selectedSuppliers.forEach((item, id) => {
            chips.push(
                chip("supplier", "Supplier", item.name, () => {
        ...
        if (els.status.value !== "") {
            chips.push(
                chip("status", "Status", els.status.value, () => {
                    els.status.value = "";
                    renderChips();
                    fetchList();
                })
    ...
    function chip(type, label, value, onRemove) {
        const id = `chip_${Math.random().toString(36).slice(2)}`;
        setTimeout(() => {
            const btn = document.getElementById(id);
            if (btn) btn.addEventListener("click", onRemove);
        }, 0);
        return `<span class="chip" data-type="${type}"><span class="text-muted">${label}:</span> ${value} <button id="${id}" aria-label="Remove"><i class="fas fa-times"></i></button></span>`;
    }
    ...
    async function fetchBrands() {
        try {
            ...
            const resp = await fetch(
                "/api/brands/autocomplete?query=&limit=1000",
                { headers: api.headers() }
            ).then(authGuard);
            const body = await resp.json();
    ...
    async function fetchSuppliers() {
        try {
            ...
            const resp = await fetch(
                "/api/suppliers/autocomplete?query=&limit=1000",
                { headers: api.headers() }
            ).then(authGuard);
            const body = await resp.json();
    ...
    function populateCategoryDropdowns() {
        console.log(
            "Populating category dropdowns with:",
            state.lookupData.categories
        );
    ...
    function populateBrandDropdowns() {
        console.log(
            "Populating brand dropdowns with:",
            state.lookupData.brands
        );
    ...
    function populateSupplierDropdowns() {
        console.log(
            "Populating supplier dropdowns with:",
            state.lookupData.suppliers
        );
        ...
        if (Array.isArray(state.lookupData.suppliers)) {
            state.lookupData.suppliers.forEach((supplier) => {
                ...
                const displayName =
                    supplier.displayName || supplier.name || supplier.id || "";
    ...
    function renderSuggestions(type, suggestions) {
        ...
        dropdown.innerHTML = suggestions
            .map(
                (item, index) => `
      <div class="kv-autocomplete__item" data-value="${
          item.id || ""
      }" data-index="${index}">
    ...
    function renderNoResults(type) {
        const dropdown = getDropdown(type);
        dropdown.innerHTML =
            '<div class="kv-autocomplete__item no-results">No results found</div>';
    }
    ...
    function handleKeyNavigation(e, type, input, dropdown) {
        ...
        switch (e.key) {
            case "ArrowDown":
                e.preventDefault();
                if (suggestions.length > 0) {
                    state.autocomplete[type].selectedIndex = Math.min(
                        state.autocomplete[type].selectedIndex + 1,
            ...
            case "ArrowUp":
                e.preventDefault();
                if (suggestions.length > 0) {
                    state.autocomplete[type].selectedIndex = Math.max(
                        state.autocomplete[type].selectedIndex - 1,
                        0
                    );
                    updateHighlightedSuggestion(type);
    ...
    function updateHighlightedSuggestion(type) {
        ...
        const items = dropdown.querySelectorAll(
            ".kv-autocomplete__item:not(.no-results)"
        );
        const selectedIndex = state.autocomplete[type].selectedIndex;
    ...
    function selectAutocompleteValue(input, type, value, text) {
        ...
        const displayName = selectedItem
            ? selectedItem.name || selectedItem.displayName || text
            : text;
    ...
    function getDropdown(type) {
        switch (type) {
            case "category":
                return els.categoryDropdown;
            case "brand":
                return els.brandDropdown;
            case "supplier":
                return els.supplierDropdown;
            default:
                return null;
        }
    }
    ...
    function getLookupData(type) {
        switch (type) {
            case "category":
                return state.lookupData.categories || [];
            case "brand":
                return state.lookupData.brands || [];
            case "supplier":
                return state.lookupData.suppliers || [];
            default:
                return [];
        }
    }
    ...
    async function initializeLookupData() {
        await Promise.all([fetchCategories(), fetchBrands(), fetchSuppliers()]);
    ...
    function setDisplayNamesFromUrl() {
        ...
        if (els.categoryId.value && els.categoryAutocomplete) {
            const category = state.lookupData.categories.find(
                (cat) => cat.id == els.categoryId.value
            );
            if (category) {
        ...
        if (els.brandId.value && els.brandAutocomplete) {
            const brand = state.lookupData.brands.find(
                (b) => b.id == els.brandId.value
            );
            if (brand) {
        ...
        if (els.supplierId.value && els.supplierAutocomplete) {
            const supplier = state.lookupData.suppliers.find(
                (s) => s.id == els.supplierId.value
            );
            if (supplier) {
    ...
    els.btnClearFilters?.addEventListener("click", () => {
        ...
        if (els.categoryAutocomplete) {
            delete els.categoryAutocomplete.dataset.selectedValue;
            els.categoryAutocomplete.value = "";
        }
        if (els.brandAutocomplete) {
            delete els.brandAutocomplete.dataset.selectedValue;
            els.brandAutocomplete.value = "";
        }
        if (els.supplierAutocomplete) {
            delete els.supplierAutocomplete.dataset.selectedValue;
    ...
    if (els.categoryAutocomplete && els.categoryDropdown && els.categoryClear) {
        initAutocomplete(
            els.categoryAutocomplete,
            els.categoryDropdown,
            els.categoryClear,
            "category"
        );
        // Initialize clear button state
        updateClearButton(
            els.categoryAutocomplete.parentElement,
            els.categoryAutocomplete.value.trim()
        );
    }
    if (els.brandAutocomplete && els.brandDropdown && els.brandClear) {
        initAutocomplete(
            els.brandAutocomplete,
            els.brandDropdown,
            els.brandClear,
            "brand"
        );
        // Initialize clear button state
        updateClearButton(
            els.brandAutocomplete.parentElement,
            els.brandAutocomplete.value.trim()
        );
    }
    if (els.supplierAutocomplete && els.supplierDropdown && els.supplierClear) {
        initAutocomplete(
            els.supplierAutocomplete,
            els.supplierDropdown,
            els.supplierClear,
            "supplier"
        );
        // Initialize clear button state
        updateClearButton(
            els.supplierAutocomplete.parentElement,
            els.supplierAutocomplete.value.trim()
        );
    }
    ...
    els.tracked?.addEventListener("change", () => {
        state.page = 0;
        fetchList();
    });
    els.btnToggleFilter?.addEventListener("click", () =>
        els.filterPanel.classList.toggle("collapsed")
    );
    els.btnCollapseFilter?.addEventListener("click", () =>
        els.filterPanel.classList.toggle("collapsed")
    );
    ...
    els.sizeSel?.addEventListener("change", () => {
        state.size = parseInt(els.sizeSel.value, 10) || 15;
        state.page = 0;
        fetchList();
    });
    els.tblBody?.addEventListener("click", (e) => {
        ...
        const id = String(rawId).includes("-exp")
        ...
        if (btn) {
            ...
            if (act === "edit") {
                fetch(`${api.base}/${id}`, { headers: api.headers() })
                    .then(authGuard)
                    .then((r) => r.json())
                    .then((body) => openModal(body.data || body))
            ...
            if (act === "toggleStatus") {
                const badge = document.querySelector(
                    `tr[data-id="${id}"] [data-col="status"] .badge-status`
                );
                ...
                fetch(`${api.base}/${id}`, {
                    ...
                    .catch((e2) => showAlert(e2.message, "danger"));
                return;
            }
        }

        // Prevent row clicks on tabs and expanded content from collapsing
        const tabClick = e.target.closest(
            '[data-bs-toggle="tab"], .nav-link, .nav-tabs'
        );
        const expandedContentClick = e.target.closest(
            ".expanded-wrapper, .tab-content, .tab-pane, .action-bar button"
        );
    ...
    els.tblHead?.addEventListener("click", (e) => {
    ...
    els.chkAll?.addEventListener("change", (e) => {
        ...
        if (e.target.checked) {
            checks.forEach((c) =>
                state.selected.add(String(c.closest("tr")?.dataset.id))
            );
        } else {
            checks.forEach((c) =>
                state.selected.delete(String(c.closest("tr")?.dataset.id))
            );
        }
    ...
    document
        ...
        ?.addEventListener("click", async () => {
            ...
            try {
                ...
                showAlert("Deleted selected products", "success");
                state.selected.clear();
                updateHeaderMode();
                fetchList();
            } catch (e2) {
                showAlert(e2.message || "Bulk delete failed", "danger");
            }
        });
    els.hdrSearch?.addEventListener(
        "input",
        debounce(() => {
            state.page = 0;
            fetchList();
    ...
    if (els.btnSaveNew)
        els.btnSaveNew.addEventListener("click", async () => {
            els.productId.value = "";
            await save();
            els.modal.show();
            document.getElementById("frm").reset();
            els.productIsTracked.checked = true;
            els.productStatus.value = "ACTIVE";
        });
    els.btnImport?.addEventListener("click", () => {
        els.importErr?.classList.add("d-none");
        if (els.fileExcel) els.fileExcel.value = "";
    ...
    els.btnImportUpload?.addEventListener("click", async () => {
        ...
        if (!f) {
            if (els.importErr) {
                els.importErr.textContent = "Please choose a file";
        ...
        fd.append("file", f);
        try {
            const resp = await fetch(`${api.base}/bulk`, {
                method: "POST",
            ...
            showAlert("Import started", "success");
            fetchList();
        } catch (e2) {
            if (els.importErr) {
                els.importErr.textContent = e2.message || "Upload failed";
                els.importErr.classList.remove("d-none");
            }
        }
    });
    ...
    function bindColToggle(input, key) {
        input?.addEventListener("change", () => {
            state.columns[key] = input.checked;
            if (!input.checked && state.sortBy === key) {
                state.sortBy = null;
                updateSortHeaders();
            }
            saveColumns();
    ...
    els.btnDownloadCsv?.addEventListener("click", () => {
        try {
            ...
            document
                .querySelectorAll("#tbl thead th[data-col]")
                .forEach((th) => {
            ...
            document
                .querySelectorAll("#tbl tbody tr[data-id]")
                .forEach((tr) => {
    ...
    function initMiniManagers() {
        ...
        // Bind Open Buttons
        document
            .getElementById("btnManageCategory")
            ?.addEventListener("click", () => openMiniManager("cat"));
        document
            .getElementById("btnManageBrand")
            ?.addEventListener("click", () => openMiniManager("brand"));
        document
            .getElementById("btnManageSupplier")
            ?.addEventListener("click", () => openMiniManager("supp"));

        // Bind Common Actions
        ["cat", "brand", "supp"].forEach((type) => {
            document
                .getElementById(`${type}-btn-add`)
                ?.addEventListener("click", () => showMiniForm(type));
            document
                .getElementById(`${type}-btn-cancel`)
                ?.addEventListener("click", () => showMiniList(type));
            document
            ...
            document.getElementById(`${type}-search`)?.addEventListener(
                "input",
                debounce((e) => {
    ...
    async function loadMiniList(type) {
        ...
        listEl.innerHTML =
            '<div class="text-center p-3 text-muted"><div class="spinner-border spinner-border-sm text-primary"></div> Loading...</div>';
        ...
        try {
            ...
            const resp = await fetch(url, { headers: api.headers() }).then(
                authGuard
            );
            const body = await resp.json();
    ...
    function renderMiniList(type, query = "") {
        ...
        if (!items.length) {
            listEl.innerHTML =
                '<div class="text-center p-3 text-muted">No items found</div>';
            return;
        }
        ...
        listEl.innerHTML = items
            .map((item) => {
                ...
                if (type === "cat") {
                    ...
                    const folder =
                        level > 0
                            ? '<i class="fas fa-level-up-alt fa-rotate-90 me-1 text-muted small"></i>'
                            : "";
                    return `
    ...
    function showMiniForm(type, data = null) {
        ...
        const isEdit = !!data;
        document.getElementById(`${type}-form-title`).textContent = isEdit
            ? `Edit ${
                  type === "cat"
                      ? "Category"
                      : type === "brand"
    ...
    async function saveMiniItem(type) {
        ...
        if (type === "cat") {
            ...
            payload.description =
                document.getElementById("cat-desc").value.trim() || null;
            payload.parentId =
                document.getElementById("cat-parent").value || null;
        } else if (type === "brand") {
            url = id ? `/api/brands/${id}` : "/api/brands";
            payload.website =
                document.getElementById("brand-website").value.trim() || null;
            payload.description =
                document.getElementById("brand-desc").value.trim() || null;
            payload.isActive = true;
        } else if (type === "supp") {
            url = id ? `/api/suppliers/${id}` : "/api/suppliers";
            payload.taxCode =
                document.getElementById("supp-tax").value.trim() || null;
            payload.contactPerson =
                document.getElementById("supp-contact").value.trim() || null;
            payload.phone =
                document.getElementById("supp-phone").value.trim() || null;
            payload.email =
                document.getElementById("supp-email").value.trim() || null;
            payload.address =
                document.getElementById("supp-address").value.trim() || null;
        }
