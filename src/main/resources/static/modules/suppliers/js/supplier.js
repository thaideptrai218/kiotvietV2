(() => {
    ...
    function updateUrl() {
        ...
        if (els.debtMin && els.debtMin.value.trim())
            p.set("debtMin", els.debtMin.value.trim());
        if (els.debtMax && els.debtMax.value.trim())
            p.set("debtMax", els.debtMax.value.trim());
        if (els.createdFrom && els.createdFrom.value)
            p.set("createdFrom", els.createdFrom.value);
        if (els.createdTo && els.createdTo.value)
            p.set("createdTo", els.createdTo.value);
    ...
    async function fetchList() {
        ...
        if (els.debtMin && els.debtMin.value.trim())
            params.set("minDebt", els.debtMin.value.trim());
        if (els.debtMax && els.debtMax.value.trim())
            params.set("maxDebt", els.debtMax.value.trim());
        if (els.createdFrom && els.createdFrom.value)
            params.set("createdFrom", els.createdFrom.value);
        if (els.createdTo && els.createdTo.value)
            params.set("createdTo", els.createdTo.value);
        ...
        renderPagination(
            ...
            data.totalPages ?? Math.ceil((data.totalElements || 0) / state.size)
        );
    ...
    function openModal(data) {
        els.modalTitle.textContent = data?.id
            ? "Edit Supplier"
            : "New Supplier";
        ...
        if (els.btnSaveNew)
            els.btnSaveNew.classList.toggle("d-none", !!data?.id);
    ...
    async function save() {
        ...
        const payload = {
            ...
            taxCode:
                (document.getElementById("modalTaxCode")?.value || "").trim() ||
                null,
            website:
        ...
        const clRaw = (document.getElementById("creditLimit")?.value ?? "")
            .toString()
            .trim();
        if (clRaw !== "") {
    ...
    function renderChips() {
        ...
        if (els.hdrSearch && els.hdrSearch.value.trim())
            chips.push(
                chip("Search", els.hdrSearch.value.trim(), () => {
                    els.hdrSearch.value = "";
                    fetchList();
                })
            );
        if (els.active.value !== "")
            chips.push(
                chip(
                    "Status",
                    els.active.value === "true" ? "Active" : "Inactive",
                    () => {
                        els.active.value = "";
                        fetchList();
                    }
        ...
        if (els.debtMin && els.debtMin.value.trim())
            chips.push(
                chip("Debt ≥", els.debtMin.value.trim(), () => {
                    els.debtMin.value = "";
                    fetchList();
                })
            );
        if (els.debtMax && els.debtMax.value.trim())
            chips.push(
                chip("Debt ≤", els.debtMax.value.trim(), () => {
                    els.debtMax.value = "";
                    fetchList();
                })
            );
        if (els.createdFrom && els.createdFrom.value)
            chips.push(
                chip("Created from", els.createdFrom.value, () => {
        ...
        if (els.createdTo && els.createdTo.value)
            chips.push(
                chip("Created to", els.createdTo.value, () => {
                    els.createdTo.value = "";
                    fetchList();
                })
            );
    ...
    function chip(label, value, onRemove) {
        ...
        return `<span class="chip"><span class="text-muted">${label}:</span> ${value} <button id="${id}" aria-label="Remove"><i class="fas fa-times"></i></button></span>`;
    }
    ...
    els.taxCode?.addEventListener("keydown", (e) => {
        if (e.key === "Enter") {
            e.preventDefault();
            state.page = 0;
            fetchList();
    ...
    document
        ...
        ?.addEventListener("click", async () => {
            ...
            if (!confirm(`Delete ${ids.length} suppliers?`)) return;
            try {
                for (const id of ids) {
                    const resp = await fetch(`${api.base}/${id}`, {
                        method: "DELETE",
                ...
                showAlert("Deleted selected suppliers", "success");
                state.selected.clear();
                updateHeaderMode();
                fetchList();
            } catch (e2) {
    ...
    els.btnImportUpload?.addEventListener("click", async () => {
        ...
        if (!fileToSend && txt) {
            ...
            fileToSend = new File([blob], "suppliers.csv", {
                type: "text/csv",
            });
        }
        if (!fileToSend) {
            if (els.importErr) {
                els.importErr.textContent = "Please choose a file or paste CSV";
    ...
    els.btnDownloadCsv?.addEventListener("click", () => {
        try {
            ...
            a.href = url;
            a.download = `suppliers_${new Date()
                .toISOString()
                .slice(0, 10)}.csv`;
